...
export class MonitoringSearchService {
	...
	private async handleStreamingSeasonPack(
		...
		try {
			...
			logger.error('[MonitoringSearch] Transaction failed for streaming season pack, cleaning up', {
				seriesId,
				seasonNumber,
			...
			for (const epData of episodeFileData) {
				try {
					...
					logger.debug('[MonitoringSearch] Cleaned up .strm file after transaction failure', {
